...
def play_matches(cpu_agents, test_agents, num_matches):
    ...
    for idx, agent in enumerate(cpu_agents):
        ...
        print("{!s:^6}{:^30}".format(idx + 1, agent.name), end="", flush=True)
...
def main():
    ...
    cpu_agents = [
        ...
        Agent(MinimaxPlayer(score_fn=open_move_score), "MiniMax_Open"),
        Agent(MinimaxPlayer(score_fn=center_score), "MiniMax_Center"),
        Agent(MinimaxPlayer(score_fn=improved_score), "MiniMax_Improved"),
        Agent(AlphaBetaPlayer(score_fn=open_move_score), "AlphaBeta_Open"),
        Agent(AlphaBetaPlayer(score_fn=center_score), "AlphaBeta_Center"),
        Agent(AlphaBetaPlayer(score_fn=improved_score), "AlphaBeta_Improved")
    ]
